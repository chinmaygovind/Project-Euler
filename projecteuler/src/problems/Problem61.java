...
public class Problem61 {
    ...
    public static void main(String[] args) {
        ...
        for (Integer start : map.keySet()){
            ...
            if (potentialCycle != null){
                ...
                System.out.println("Their polygonal sides are: " + potentialCycle.stream().map(Problem61::getGroup).collect(Collectors.toList()));//hehehe
    ...
    private static ArrayList<Integer> findCycle(ArrayList<Integer> path){
        ...
        for (Integer potentialNext : map.get(path.get(path.size() - 1))) {
            ...
            if (path.stream().map(Problem61::getGroup).collect(Collectors.toList()).contains(getGroup(potentialNext))) continue;
